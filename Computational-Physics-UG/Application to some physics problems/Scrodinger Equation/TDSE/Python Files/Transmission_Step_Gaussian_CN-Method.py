#Solution of TDSE for a free gaussian wavepacket undergoing transmission through step potential barrier using FTCS Crank Nicolson Method
...
plt.plot(xs,us,color='lime',label=r'Initial Wavepacket $\psi(x,0)$')
line1,=plt.plot(xs,us,color='green',label=r'Evolving Wavepacket $\psi(x,t)$')       #Unpacking from list

plt.plot(xs,np.abs(us)**2,color='lightblue',label=r'$|\psi(x,0)|^2$')
line2,=plt.plot(xs,np.abs(us)**2,color='blue',label=r'$|\psi(x,t)|^2$')             #Unpacking from list
...
plt.savefig('/home/tux_blue/Documents/GitHub/Computational-Physics-UG/Application to some physics problems/Scrodinger Equation/TDSE/Transmission-Step_Gaussian_CN-Method.png')    
plt.show()
