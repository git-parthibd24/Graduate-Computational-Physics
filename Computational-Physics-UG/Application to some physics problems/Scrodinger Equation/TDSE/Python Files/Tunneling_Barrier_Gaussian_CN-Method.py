#Solution of TDSE for a free gaussian wavepacket undergoing tunneling using FTCS Crank Nicolson Method
...
k=8.0           #This is the particle's initial energy : E=k**2/2
...
#Applied potential
Vs=np.where((xs[1:-1]>=5) & (xs[1:-1]<=6),35,0)     #Only taking inside points which will be updated by M matrix. In NumPy, multiplying a 2D identity matrix by a 1D array broadcasts that array along the diagonal of the identity matrix.                              
...
plt.plot(xs,us,color='skyblue',label=r'Initial Wavepacket $\psi(x,0)$')
line1,=plt.plot(xs,us,color='steelblue',label=r'Evolving Wavepacket $\psi(x,t)$')       #Unpacking from list

plt.plot(xs,np.abs(us)**2,color='khaki',label=r'$|\psi(x,0)|^2$')
line2,=plt.plot(xs,np.abs(us)**2,color='gold',label=r'$|\psi(x,t)|^2$')             #Unpacking from list
...
plt.savefig('/home/tux_blue/Documents/GitHub/Computational-Physics-UG/Application to some physics problems/Scrodinger Equation/TDSE/Tunneling-Barrier_Gaussian_CN-Method.png')    
plt.show()
