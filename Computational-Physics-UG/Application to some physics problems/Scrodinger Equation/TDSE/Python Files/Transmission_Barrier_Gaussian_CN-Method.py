#Solution of TDSE for a free gaussian wavepacket undergoing transmission through a finite width potential barrier using FTCS Crank Nicolson Method
...
k=10.0          #This is the particle's initial energy : E=k**2/2
...
#Applied potential
Vs=np.where((xs[1:-1]>=5) & (xs[1:-1]<=15),35,0)     #Only taking inside points which will be updated by M matrix. In NumPy, multiplying a 2D identity matrix by a 1D array broadcasts that array along the diagonal of the identity matrix.                              
...
plt.plot(xs,us,color='pink',label=r'Initial Wavepacket $\psi(x,0)$')
line1,=plt.plot(xs,us,color='crimson',label=r'Evolving Wavepacket $\psi(x,t)$')       #Unpacking from list

plt.plot(xs,np.abs(us)**2,color='skyblue',label=r'$|\psi(x,0)|^2$')
line2,=plt.plot(xs,np.abs(us)**2,color='darkblue',label=r'$|\psi(x,t)|^2$')             #Unpacking from list
...
plt.savefig('/home/tux_blue/Documents/GitHub/Computational-Physics-UG/Application to some physics problems/Scrodinger Equation/TDSE/Transmission-Barrier_Gaussian_CN-Method.png')    
plt.show()
