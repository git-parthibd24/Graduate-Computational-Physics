#Fourier series for sawtooth waveform using quad() in [-L/2,L/2] interval
...
Ls=eval(input('Enter the period of the sawtooth wave : '))
m=2*np.pi/Ls                                              #Periodicity of the function 
...
fc=lambda x: sawtooth(m*x)*np.cos(2*np.pi*i*x/L)
fs=lambda x: sawtooth(m*x)*np.sin(2*np.pi*i*x/L)
...
plt.title('Fourier Series for sawtooth waveform in [-L/2,L/2]')
