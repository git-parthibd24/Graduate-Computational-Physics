#Fourier series for triangular waveform using quad() in [-L/2,L/2] interval
...
fc=lambda x: sawtooth(m*x,width=0.5)*np.cos(2*np.pi*i*x/L)
fs=lambda x: sawtooth(m*x,width=0.5)*np.sin(2*np.pi*i*x/L)
