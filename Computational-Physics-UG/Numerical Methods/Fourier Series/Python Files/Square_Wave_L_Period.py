#Fourier series for square waveform using quad() in [-L/2,L/2] interval
...
fc=lambda x: square(m*x)*np.cos(2*np.pi*i*x/L)
fs=lambda x: square(m*x)*np.sin(2*np.pi*i*x/L)
